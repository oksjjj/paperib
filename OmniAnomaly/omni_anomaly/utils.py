...
# Legacy flat layout (pre-THOC alignment). Prefer shared paperib data/{SMAP,MSL,SMD}/.
_LEGACY_PREFIX = 'processed'
# omni_anomaly/utils.py → OmniAnomaly → paperib repo root
_PAPERIB_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))


def paperib_root():
    """Repo root shared by OmniAnomaly, AnomalyTransformer, …"""
    return _PAPERIB_ROOT


def data_dir_for(dataset):
    """
    Prepared pickle directory for a dataset (shared across models).

    Layout under the paperib repo root:
      ``SMAP`` / ``MSL`` → ``data/{name}/pickle``
      ``machine-*``     → ``data/SMD/pickle``
      ``P*``            → ``data/ib_data/pickle``
    """
    name = str(dataset)
    if name in ('SMAP', 'MSL'):
        candidates = [
            os.path.join(_PAPERIB_ROOT, 'data', name, 'pickle'),
            os.path.join(_PAPERIB_ROOT, 'data', name),  # legacy flat
            _LEGACY_PREFIX,
        ]
    elif name.startswith('machine'):
        candidates = [
            os.path.join(_PAPERIB_ROOT, 'data', 'SMD', 'pickle'),
            os.path.join(_PAPERIB_ROOT, 'data', 'SMD'),  # legacy flat
            _LEGACY_PREFIX,
        ]
    elif name.startswith('P'):
        candidates = [
            os.path.join(_PAPERIB_ROOT, 'data', 'ib_data', 'pickle'),
            os.path.join(_PAPERIB_ROOT, 'data', 'ib_data', 'paperib'),  # legacy
            _LEGACY_PREFIX,
        ]
    else:
        candidates = [_LEGACY_PREFIX]

    train_name = f'{name}_train.pkl'
    for path in candidates:
        if os.path.isfile(os.path.join(path, train_name)):
            return path
    return candidates[0]


def raw_dir_for(dataset):
    """Raw download directory: ``data/SMD/raw``, ``data/SMAP/raw``, …"""
    name = str(dataset)
    if name.startswith('machine'):
        name = 'SMD'
    if name not in ('SMD', 'SMAP', 'MSL'):
        raise ValueError(f'no raw dir for dataset {dataset!r}')
    return os.path.join(_PAPERIB_ROOT, 'data', name, 'raw')
