...
공유 경로 (paperib repo root):
  data/{SMD,SMAP,MSL}/raw → data/{…}/pickle
...
def parse_args():
    ...
    p.add_argument(
        ...
        help='Raw data root (default: data/{dataset}/raw)',
    )
    p.add_argument(
        '--output_dir', default=None,
        help='Pickle output dir (default: data/{dataset}/pickle)',
    )
    return p.parse_args()
...
def main():
    ...
    raw_dir = args.raw_dir or os.path.join('data', args.dataset, 'raw')
    output_dir = args.output_dir or os.path.join('data', args.dataset, 'pickle')
    ...
    print(f'\nDone. Prepared files under {os.path.join(_PAPERIB_ROOT, output_dir)}/')
