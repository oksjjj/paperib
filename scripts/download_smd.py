...
# -*- coding: utf-8 -*-
"""
Download SMD (Server Machine Dataset) into paperib ``data/SMD/raw/``.
...
OUTPUT_DIR = os.path.join('data', 'SMD', 'raw')
...
def download_smd(force=False):
    ...
    try:
        ...
        print(
            '  mkdir -p data/SMD && mv _omni_tmp/ServerMachineDataset '
            f'{OUTPUT_DIR} && rm -rf _omni_tmp',
            file=sys.stderr,
        )
        print(
            '(run the git commands from the paperib repo root)',
            file=sys.stderr,
        )
        sys.exit(1)
